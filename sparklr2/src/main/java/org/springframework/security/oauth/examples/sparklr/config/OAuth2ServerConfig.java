...
public class OAuth2ServerConfig {
	...
	protected static class UiResourceConfiguration extends WebSecurityConfigurerAdapter {
		...
		protected void configure(HttpSecurity http) throws Exception {
			...
			.and()
				.authorizeRequests()
				.antMatchers("/photos").access("hasRole('ROLE_CLIENT')")
				.antMatchers("/photos/trusted/**").access("hasRole('ROLE_CLIENT')")
				.antMatchers("/photos/user/**").access("hasRole('ROLE_CLIENT')")
				.antMatchers("/photos/**").access("hasRole('ROLE_CLIENT')");
			// @formatter:on
		}
	}
		...
		public void configure(HttpSecurity http) throws Exception {
			...
			.and()
				.authorizeRequests()
					...
					.antMatchers("/userinfo").access("#oauth2.hasScope('trust')")
	...
	protected static class AuthorizationServerConfiguration extends AuthorizationServerConfigurerAdapter {
		...
		public void configure(OAuth2AuthorizationServerConfigurer oauthServer) throws Exception {
			oauthServer.tokenStore(tokenStore).authenticationManager(authenticationManager).realm("sparklr2/client");
		}
